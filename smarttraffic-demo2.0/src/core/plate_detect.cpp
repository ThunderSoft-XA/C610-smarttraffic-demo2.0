...
namespace easypr {
  ...
  int CPlateDetect::plateDetect(Mat src, std::vector<CPlate> &resultVec, int type,
    ...
    std::vector<CPlate> SNPE_Plates;
    SNPE_Plates.reserve(16);
...
#pragma omp section
      {
        if (!type || type & PR_DETECT_SNPE) {
          m_plateLocate->plateSnpeLocate(src, SNPE_Plates, img_index);
        }
      }
    }
    ...
    for (auto plate : mser_Plates) {
     ...
     for (auto plate : SNPE_Plates) {
      plate.setPlateLocateType(SNPE);
      all_result_Plates.push_back(plate);
    }
