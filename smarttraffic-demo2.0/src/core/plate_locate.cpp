...
//! SNPE plate locate
int CPlateLocate::plateSnpeLocate(Mat src, vector<CPlate> &candPlates, int img_index) {

  SNPEPlate cardPlate;
  vector<CPlate> plates_snpe;
  plates_snpe.reserve(64);
  if(!cardPlate.init()) {
      return -1;
  }
  cardPlate.setSrcMat(src);
  if(cardPlate.SnpeSearch() != 0 ) {
    return -1;
  }
  CPlate snpeCPlate;
  for(int i = 0; i < cardPlate.roiMatVec.size(); i++) {
    snpeCPlate.setPlateMat(cardPlate.roiMatVec[i]);
    candPlates.push_back(snpeCPlate);
  }
...
int CPlateLocate::plateLocate(Mat src, vector<Mat> &resultVec, int index) {
  ...
  plateSnpeLocate(src, all_result_Plates, index);
